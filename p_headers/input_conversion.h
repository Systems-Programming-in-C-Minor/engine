...
SDL_Scancode parse_numeric_keys_to_scancode(Key key){
    switch(key){
        case NUM_0:
            return static_cast<SDL_KeyCode>(SDLK_0);
        case NUM_1:
...
SDL_KeyCode parse_alpha_keys_to_scancode(Key key){
    switch(key){
        case A:
            return static_cast<SDL_KeyCode>(SDLK_a);
        case B:
...
SDL_KeyCode parse_function_keys_to_scancode(Key key){
    switch(key){
        case UP:
            return static_cast<SDL_KeyCode>(SDLK_UP);
        case LEFT:
...
SDL_KeyCode parse_key_to_scancode(Key key){
    std::string strCheck = ""+key;
    if(strCheck.length() == 1)
        return parse_alpha_keys(key);
