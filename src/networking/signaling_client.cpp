...
/// Implementation of ISignalingClient
class CSignalingClient : public ISignalingClient {
    ...
    struct ConnectionSignaling : ISteamNetworkingConnectionSignaling {
        ...
        bool SendSignal(HSteamNetConnection hConn, const SteamNetConnectionInfo_t &info, const void *pMsg,
                        int cbMsg) override {
            // Silence warnings
            (void) info;
            (void) hConn;
            ...
            signal.push_back(' ');
            for (const auto *p = (const uint8_t *) pMsg; cbMsg > 0; --cbMsg, ++p) {
    ...
    void Connect() {
        ...
        // And immediate send our greeting.  This just puts in the buffer, and
        // it will go out once the socket connects.
        Send(m_sGreeting);
    }

public:
    CSignalingClient(const sockaddr *adrServer, size_t adrServerSize,
        ...
        auto identitySelf = SteamNetworkingIdentity();
        identitySelf.Clear();
    ...
    void poll() override {
        ...
        if (m_sock == INVALID_SOCKET) {
            ...
            for (;;) {
                char buf[256];
                const auto r = recv(m_sock, buf, sizeof(buf), 0);
                if (r == 0)
                    break;
                if (r < 0) {
                    ...
                    if (!IgnoreSocketError(e)) {
                        printf(
                                "Failed to recv from trivial signaling server.  recv() returned %ld, errno=%d.  Closing and restarting connection\n",
                                r, e);
                        CloseSocket();
                    }
                    break;
        ...
        if (m_sock != INVALID_SOCKET) {
            while (!m_queueSend.empty()) {
                ...
                int l = (int) s.length();
                auto r = send(m_sock, s.c_str(), l, 0);
                ...
                if (r == l) {
                    ...
                    printf(
                            "Failed to send %d bytes to trivial signaling server.  send() returned %ld, errno=%d.  Closing and restarting connection.\n",
                            l, r, GetSocketError());
                    CloseSocket();
                    break;
                }
        ...
        for (;;) {
            ...
            if (spc != std::string::npos && spc < l) {
                ...
                // Set up a context object that can respond if this signal is a connection request.
