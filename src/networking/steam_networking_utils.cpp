...
#include "steam_networking_utils.hpp"
    ...
    switch (pInfo->m_info.m_eState) {
        ...
        case k_ESteamNetworkingConnectionState_Connected:
            ...
            assert(pInfo->m_hConn ==
                   g_hConnection); // We don't initiate or accept any other connections, so this should be out own connection
            printf("[%s] connected\n", pInfo->m_info.m_szConnectionDescription);
            break;
...
int run(int argc, const char **argv) {
    auto identityLocal = SteamNetworkingIdentity();
    identityLocal.Clear();
    auto identityRemote = SteamNetworkingIdentity();
    ...
    for (int idxArg = 1; idxArg < argc; ++idxArg) {
        ...
        auto GetArg = [&]() -> const char * {
            if (idxArg + 1 >= argc) {
                printf("Expected argument after %s", pszSwitch);
                exit(1);
            }
            return argv[++idxArg];
        };
        auto ParseIdentity = [&](SteamNetworkingIdentity &x) {
            ...
            if (!x.ParseString(pszArg)) {
                printf("'%s' is not a valid identity string", pszArg);
                exit(1);
            }
        };
        ...
        if (!strcmp(pszSwitch, "--identity-local"))
            ...
            printf("Unexpected command line argument '%s'", pszSwitch);
            exit(1);
        }
    }

    if (g_eTestRole == k_ETestRole_Undefined) {
        printf("Must specify test role (--server, --client, or --symmetric");
        exit(1);
    }
    if (identityLocal.IsInvalid()) {
        printf("Must specify local identity using --identity-local");
        exit(1);
    }
    if (identityRemote.IsInvalid() && g_eTestRole != k_ETestRole_Server) {
        printf("Must specify remote identity using --identity-remote");
        exit(1);
    }
    ...
    ISignalingClient *pSignaling = CreateSignalingClient(pszTrivialSignalingService,
    ...
    if (g_eTestRole == k_ETestRole_Server) {
        ...
        printf("Creating listen socket in symmetric mode, local virtual port %d\n", g_nVirtualPortLocal);
        auto opt = SteamNetworkingConfigValue_t();
        opt.SetInt32(k_ESteamNetworkingConfig_SymmetricConnect,
                     1); // << Note we set symmetric mode on the listen socket
    ...
    if (g_eTestRole != k_ETestRole_Server) {
        ...
        if (g_eTestRole == k_ETestRole_Symmetric) {
            ...
            vecOpts.push_back(opt);
            printf("Connecting to '%s' in symmetric mode, virtual port %d, from local virtual port %d.\n",
        ...
        g_hConnection = SteamNetworkingSockets()->ConnectP2PCustomSignaling(pConnSignaling, &identityRemote,
                                                                            g_nVirtualPortRemote, (int) vecOpts.size(),
                                                                            vecOpts.data());
